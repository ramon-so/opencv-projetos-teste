...
for x in range(0, img1.shape[0]):
    for y in range(0, img1.shape[1]):
        if((img1[x, y] - img2[x, y]) < 0):
            img4[x, y] = 0
        else:
            img4[x, y] = img1[x, y] - img2[x, y]

cv2.imshow('sub opencv', img3)
cv2.moveWindow("sub opencv", img2.shape[1] + 10, img1.shape[0] + 40) 

cv2.imshow('sub for unit8', img4)
cv2.moveWindow("sub for unit8", 0, img1.shape[0] + 40) 

for x in range(0, img1.shape[0]):
    for y in range(0, img1.shape[1]):
        if((int(img1[x, y]) - int(img2[x, y])) < 0):
            img5[x, y] = 0
        else:
            img5[x, y] = int(img1[x, y]) - int(img2[x, y])

cv2.imshow('sub for int', img5)
cv2.moveWindow("sub for int", 0, img1.shape[0] *2 + 80) 
